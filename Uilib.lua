    ...
    -- Fixed minimize animation synchronization
    function Library:Minimize()
        ...
        Tween(self.Title, {TextTransparency = 1}, animTime)
        Tween(self.CloseButton, {TextTransparency = 1}, animTime)
        Tween(self.MinimizeButton, {TextTransparency = 1}, animTime)
        Tween(self.TopDivider, {BackgroundTransparency = 1}, animTime)
        Tween(self.VerticalDivider, {BackgroundTransparency = 1}, animTime)
        
        -- Minimize main frame
        Tween(self.MainFrame, {Size = UDim2.new(0, 0, 0, 0), BackgroundTransparency = 1}, animTime)
        Tween(self.EdgeGlow, {ImageTransparency = 1}, animTime)
        Tween(self.BackgroundImage, {ImageTransparency = 1}, animTime)
        
        -- Hide active functions frame
        if self.ActiveFunctionsFrame then
            Tween(self.ActiveFunctionsFrame, {Position = UDim2.new(1, 50, 0, 20)}, animTime)
        end
    ...
    function Library:Restore()
        ...
        -- Reset transparencies before animating
        ...
        -- Restore active functions frame
        if self.ActiveFunctionsFrame then
            Tween(self.ActiveFunctionsFrame, {Position = UDim2.new(1, -220, 0, 20)}, 0.3)
        end
    end
    
    -- New Container Element (big button that rolls down)
    function Library:CreateContainer(section, config)
        config = config or {}
        local container = {}
        ...
        container.Frame:SetAttribute("OriginalTransparency", self.ButtonDarkness)
    ...
    function Library:SetButtonDarkness(darkness)
        ...
        -- Update all existing buttons with their original transparencies
        for _, descendant in pairs(self.ScreenGui:GetDescendants()) do
            ...
            elseif descendant:IsA("TextBox") and descendant:GetAttribute("OriginalBackgroundTransparency") then
                -- Special handling for textboxes which have different base transparency
                local baseTransparency = 0.3
                descendant.BackgroundTransparency = baseTransparency
                descendant:SetAttribute("OriginalBackgroundTransparency", baseTransparency)
            end
        end
    end
    ...
    -- Methods to customize section headers
    function Library:SetSectionHeaderStyle(config)
        if config.Size then
            self.SectionHeaderSize = config.Size
        end
        if config.Alignment then
            self.SectionHeaderAlignment = config.Alignment
        end
        if config.Font then
            self.SectionHeaderFont = Fonts[config.Font] or config.Font
        end
        if config.Color then
            self.SectionHeaderColor = config.Color
        end
        if config.YOffset then
            self.SectionHeaderYOffset = config.YOffset
        end
        ...
        for _, section in pairs(self.Sections) do
            if section.Header then
                if config.Size then
                    section.Header.TextSize = config.Size
                end
                if config.Alignment then
                    section.Header.TextXAlignment = config.Alignment
                    -- Update underline position
                    if section.HeaderUnderline then
                        section.HeaderUnderline.Position = UDim2.new(
                            config.Alignment == Enum.TextXAlignment.Center and 0.25 or 0, 
                            0, 0, 35
                        )
                    end
                end
                if config.Font then
                    section.Header.Font = self.SectionHeaderFont
                end
                if config.Color then
                    section.Header.TextColor3 = config.Color
                    if section.HeaderUnderline then
                        section.HeaderUnderline.BackgroundColor3 = config.Color
                    end
                end
                if config.YOffset then
                    section.Header.Position = UDim2.new(0, 10, 0, config.YOffset)
                    -- Update content position
                    local yOffset = self.SectionHeaderEnabled and (config.YOffset + 40) or 5
                    local ySize = self.SectionHeaderEnabled and (config.YOffset + 45) or 10
                    section.Content.Position = UDim2.new(0, 10, 0, yOffset)
                    section.Content.Size = UDim2.new(1, -20, 1, -ySize)
                end
            end
        end
    end
    ...
    -- New UI Settings Section Creator
    function Library:CreateUISettings()
        local settingsSection = self:CreateSection("UI Settings")
        
        -- Theme Dropdown
        self:CreateDropdown(settingsSection, {
            Text = "Theme",
        ...
        self:CreateDropdown(settingsSection, {
            Text = "Font",
            Options = {"Ubuntu", "Gotham", "GothamBold", "SourceSans", "SourceSansBold", "Code", "Highway", "SciFi", "Arial", "ArialBold"},
            Default = "Ubuntu",
        ...
        self:CreateSlider(settingsSection, {
        ...
        -- Section Header Customization Container
        local headerContainer = self:CreateContainer(settingsSection, {
            Text = "Section Header Style"
        })
        
        -- Header Size Slider
        headerContainer.AddElement(self.CreateSlider, {
            Text = "Header Size",
            Min = 14,
            Max = 30,
            Default = self.SectionHeaderSize,
            Callback = function(value)
                self:SetSectionHeaderStyle({Size = value})
            end
        })
        
        -- Header Color Dropdown
        headerContainer.AddElement(self.CreateDropdown, {
            Text = "Header Color",
            Options = {"Theme Accent", "White", "Red", "Green", "Blue", "Yellow"},
            ...
            Callback = function(color)
                ...
                self:SetSectionHeaderStyle({Color = colors[color]})
            end
        })
        ...
        headerContainer.AddElement(self.CreateDropdown, {
            ...
            Callback = function(alignment)
                ...
                self:SetSectionHeaderStyle({Alignment = alignments[alignment]})
            end
        })
        
        -- Header Y Offset Slider
        headerContainer.AddElement(self.CreateSlider, {
            Text = "Header Y Offset",
            Min = 0,
            Max = 20,
            Default = self.SectionHeaderYOffset,
            Callback = function(value)
                self:SetSectionHeaderStyle({YOffset = value})
            end
        })
    ...
    return Library {
        ...
        container.Button = CreateInstance("TextButton", {
            Size = UDim2.new(1, -30, 0, 35),
            Position = UDim2.new(0, 10, 0, 0),
            BackgroundTransparency = 1,
            BorderSizePixel = 0,
            Text = config.Text or "Container",
            TextColor3 = self.Theme.Text,
            TextSize = 14,
            Font = self.Font,
            TextXAlignment = Enum.TextXAlignment.Left
        ...
        container.Arrow = CreateInstance("TextLabel", {
            Size = UDim2.new(0, 20, 0, 20),
        ...
        container.ContentFrame = CreateInstance("Frame", {
            Size = UDim2.new(1, -20, 0, 0),
            Position = UDim2.new(0, 10, 0, 35),
            BackgroundTransparency = 1,
            BorderSizePixel = 0,
        ...
        container.Content = CreateInstance("ScrollingFrame", {
            Size = UDim2.new(1, 0, 1, 0),
            Position = UDim2.new(0, 0, 0, 0),
            BackgroundTransparency = 1,
            BorderSizePixel = 0,
        ...
        container.Button.MouseButton1Click:Connect(function()
            ...
            if container.Open then
                ...
                contentHeight = math.min(contentHeight + 10, 200) -- Max height
                
                -- Animate opening
                Tween(container.Arrow, {Rotation = 180}, 0.3, Enum.EasingStyle.Back)
                Tween(container.ContentFrame, {Size = UDim2.new(1, -20, 0, contentHeight)}, 0.3, Enum.EasingStyle.Back)
                Tween(container.Frame, {Size = UDim2.new(1, 0, 0, 35 + contentHeight)}, 0.3, Enum.EasingStyle.Back)
            else
                -- Animate closing
                Tween(container.Arrow, {Rotation = 0}, 0.3, Enum.EasingStyle.Back)
                Tween(container.ContentFrame, {Size = UDim2.new(1, -20, 0, 0)}, 0.3, Enum.EasingStyle.Back)
                Tween(container.Frame, {Size = UDim2.new(1, 0, 0, 35)}, 0.3, Enum.EasingStyle.Back)
            end
        ...
        container.Frame.MouseEnter:Connect(function()
            Tween(container.Frame, {BackgroundTransparency = self.ButtonDarkness - 0.2}, 0.2)
            Mouse.Icon = "rbxasset://SystemCursors/Hand"
        end)
        
        container.Frame.MouseLeave:Connect(function()
            Tween(container.Frame, {BackgroundTransparency = self.ButtonDarkness}, 0.2)
            Mouse.Icon = ""
        end)
        
        -- Method to add elements to container
        container.AddElement = function(elementFunction, ...)
            local element = elementFunction(self, {Content = container.Content}, ...)
            ...
            if container.Open then
                ...
                contentHeight = math.min(contentHeight + 10, 200)
                
                Tween(container.ContentFrame, {Size = UDim2.new(1, -20, 0, contentHeight)}, 0.2)
                Tween(container.Frame, {Size = UDim2.new(1, 0, 0, 35 + contentHeight)}, 0.2)
            end
